...
public:
	...
	//可以把中间结果dump 出来， 然后下次load 进去，继续计算
	//params_buf_len = 192 
	int DumpParams(char *params_buf, UINT_32* params_buf_len);
	int LoadParams(char *params_buf, UINT_32 params_buf_len);

	//获取中间结果sha，直接将中间结果的5个分量进行合并，并返回已完成 sha的数据字节数
	void ReportTempHash(char *szReport, unsigned long long & datalen);
	//将中间sha加载进去，继续计算，中间sha为二进制20字节，datalen表示已完成sha的数据量
	void LoadTempHash(const char *tempBinSha, unsigned long long datalen);
